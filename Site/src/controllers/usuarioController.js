...
function cadastrar(req, res) {
    ...
    if (email == undefined) {
        ...
        res.status(400).send("Sua nick está undefined!");
    } else if(senha == undefined) {
        res.status(400).send("Sua senha está undefined!");
    } else if(analise == undefined) {
        res.status(400).send("Sua analise está undefined!");
    } else {

        // Passe os valores como parâmetro e vá para o arquivo usuarioModel.js
        usuarioModel.cadastrar(email, nome, sobrenome, nick, senha, analise)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
